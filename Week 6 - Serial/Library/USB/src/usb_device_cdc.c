...
uint8_t getUSBUSART_FIFO(FIFOPtr pInput)
{
    cdc_rx_len = 0;
    ...
    if(!USBHandleBusy(CDCDataOutHandle))
    {
         ...
         * the actual number of BYTEs received.
         */
        UINT16 len = USBHandleGetLength(CDCDataOutHandle);
	if(len > pInput->mLength)
            len = pInput->mLength;
         ...
         */
        for(cdc_rx_len = 0; cdc_rx_len < len; cdc_rx_len++)
	    FIFO_Put(&pInput, cdc_data_rx[cdc_rx_len]);
...
void putUSBUSART_FIFO(FIFOPtr pInput)
{
    /*
//     * User should have checked that cdc_trf_state is in CDC_TX_READY state
//     * before calling this function.
