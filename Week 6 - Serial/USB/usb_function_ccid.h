...
extern unsigned char usbCcidBulkOutEndpoint[100];	//User application buffer for receiving and holding OUT packets sent from the host
extern unsigned char usbCcidBulkInEndpoint[100];		//User application buffer for sending IN packets to the host

/** Section: PUBLIC PROTOTYPES **********************************************/
void USBCheckCCIDRequest(void);
void USBCCIDInitEP(void);
void USBCCIDBulkInService(void);
void USBCCIDSendDataToHost(UINT8Ptr pData, WORD len);

/** Section: STRUCTURES **********************************************/
typedef union {
    UINT8 CCID_BulkOutBuffer[271];
    UINT8 CCID_BulkInBuffer[267]; 
} USB_CCID_BUFFER;  
