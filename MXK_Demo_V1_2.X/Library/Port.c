...
void		Port_Set	    (PortPtr pInput, UINT8 pValue)
{
	...
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 1u) << 2u;
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 2u) << 4u;
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 3u) << 6u;
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 4u) << 8u;
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 5u) << 10u;
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 6u) << 12u;
	TypeActual |= (ePinType)Word_GetPair(&pInput->mType, 7u) << 14u;
